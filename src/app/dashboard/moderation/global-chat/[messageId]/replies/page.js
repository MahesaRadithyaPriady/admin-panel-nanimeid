...
import { ArrowLeft, Globe2, Trash2, RefreshCw, Search } from 'lucide-react';
import { useSession } from '@/hooks/useSession';
import { getSession } from '@/lib/auth';
import { listModerationGlobalChatReplies, softDeleteGlobalChat } from '@/lib/api';
...
export default function ModerationGlobalChatRepliesPage() {
  ...
  const loadReplies = async (opts = {}) => {
    ...
    try {
      ...
      const data = await listModerationGlobalChatReplies({
        token,
        messageId,
        page,
        limit,
      ...
      toast.error(err?.message || 'Gagal memuat global chat replies');
    } finally {
  ...
  useEffect(() => {
    if (!user || !canAccess || !messageId) return;
    loadReplies();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user, canAccess, messageId, page, limit, isDeleted]);
  ...
  const doSoftDelete = async (row) => {
    ...
    try {
      ...
      toast.success('Reply global chat berhasil di-soft-delete');
      await loadReplies();
    } catch (err) {
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="font-extrabold text-2xl flex items-center gap-2">
              <Globe2 className="size-6" /> Replies Global Chat
            </div>
            <div className="text-xs text-zinc-500 font-bold mt-1">Message ID: {String(messageId || '-')}</div>
          </div>
        </div>
      </div>
      ...
      {parent ? (
        <Panel
          ...
          icon={Globe2}
          description="Ini parent/root message yang kamu buka."
        >
          <div className="space-y-2">
            <div className="font-extrabold text-sm">#{parent?.id}</div>
                  ...
                  >
                    ...
                    {parentIsSticker ? (
                      <div className="mt-3">
                        ...
                        <StickerPreview sticker={parent?.sticker} />
                      </div>
                    ) : null}
                  </div>
                );
